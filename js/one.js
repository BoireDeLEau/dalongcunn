// JavaScript Document
window.onload=function(){
	alert("进入首页请单击左上角“大龙村”!");
}
//标题栏下拉菜单
function change(myid,mode){
	document.getElementById(myid).style.display=mode;
	if(mode == 'block'){//显示下拉菜单
		//设置鼠标划过的背景颜色
		document.getElementById(myid).parentNode.style.borderBottom="none";
	}
	else{
		//当不显示下拉列表时，鼠标划过的背景颜色
		document.getElementById(myid).parentNode.style.backgroundColor="";
		document.getElementById(myid).parentNode.style.border="";
	}		
}

document.addEventListener('DOMContentLoaded', () => {  
    const publishResumeButton = document.querySelector('.publish-resume');  
    const publishJobButton = document.querySelector('.publish-job');  
  
    publishResumeButton.addEventListener('click', () => {  
        alert('正在跳转到发布简历页面...');  
        // 实际上可以执行跳转到发布简历页面的逻辑，如 window.location.href = '...';  
    });  
  
    publishJobButton.addEventListener('click', () => {  
        alert('正在跳转到发布职位页面...');  
        // 实际上可以执行跳转到发布职位页面的逻辑，如 window.location.href = '...';  
    });  
});